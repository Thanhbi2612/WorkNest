-- ============================================
  ...
  -- Sample tasks
  INSERT INTO tasks (title, description, status, priority, due_date, assignee_id, creator_id, creator_type, project_id, created_at, updated_at)
  VALUES
  ('Setup Development Environment', 'Install Node.js, PostgreSQL, Docker', 'completed', 'high', NOW() - INTERVAL '5 days', 1, 1, 'admin', 1, NOW() - INTERVAL '7 days', NOW()),       
  ('Design Database Schema', 'Create ERD and implement PostgreSQL schema', 'completed', 'high', NOW() - INTERVAL '3 days', 1, 1, 'admin', 1, NOW() - INTERVAL '6 days', NOW()),       
  ('Implement Authentication', 'JWT + Google OAuth integration', 'in_progress', 'high', NOW() + INTERVAL '2 days', 1, 1, 'admin', 1, NOW() - INTERVAL '4 days', NOW()),
  ('Build Chat Feature', 'Real-time chat with Socket.io', 'in_progress', 'medium', NOW() + INTERVAL '5 days', 2, 1, 'admin', 1, NOW() - INTERVAL '3 days', NOW()),
  ('Create Dashboard', 'Admin dashboard with statistics', 'not_started', 'medium', NOW() + INTERVAL '7 days', 2, 1, 'admin', 1, NOW() - INTERVAL '2 days', NOW());
